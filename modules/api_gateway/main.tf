...
# Create the POST method for /approve/{request_id} (changed from GET to POST)
resource "aws_api_gateway_method" "approve_post" {
  rest_api_id   = aws_api_gateway_rest_api.this.id
  resource_id   = aws_api_gateway_resource.approve_id.id
  http_method   = "POST"  # Changed to POST to match Lambda handler
  authorization = "NONE"
  request_parameters = {
    "method.request.path.request_id" = true  # Require the request_id path parameter
  }
}

# Integrate the POST /approve/{request_id} with a Lambda function using AWS_PROXY
